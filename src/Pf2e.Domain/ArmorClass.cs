...
{
    /// <summary>
    /// 10 + armour proficiency + Dex (capped by the armour) + stacked modifiers. The armour's
    /// own item bonus is passed as an item <see cref="Modifier"/> so it competes with other item
        ...
        RuleOptions options = default)
    {
        ...
        return Stacking.Resolve(baseValue, StatTarget.ArmorClass, modifiers);
    }
}
